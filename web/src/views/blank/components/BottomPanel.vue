...
const SIDEBAR_LEFT_OFFSET = 8 + 320 + 8;

const panelStyle = computed(() => {
  const left = props.leftCollapsed ? 0 : SIDEBAR_LEFT_OFFSET;
  const right = props.rightCollapsed ? 0 : SIDEBAR_LEFT_OFFSET;
...
<template>
  <div class="bottom-panel" :class="{ collapsed: bottomCollapsed }" :style="panelStyle">
    <div class="bottom-panel-handle" @click="emit('toggle')">
      ...
      <span>{{ bottomCollapsed ? '展开工具台' : '收起工具台' }}</span>
    </div>

    <div v-show="!bottomCollapsed" class="bottom-panel-body">
      <div class="panel-summary">
        <div class="summary-card">
          <div class="summary-kicker">调试总览</div>
          <div class="summary-value">{{ logRows.length }}</div>
          <div class="summary-meta">条运行日志</div>
        </div>
        <div class="summary-card danger" v-if="errorCount">
          <div class="summary-kicker">错误提示</div>
          <div class="summary-value">{{ errorCount }}</div>
          <div class="summary-meta">条错误或异常输出</div>
        </div>
        <div class="summary-card" v-else>
          <div class="summary-kicker">运行状态</div>
          <div class="summary-value">OK</div>
          <div class="summary-meta">当前未发现错误日志</div>
        </div>
        <div class="summary-card">
          <div class="summary-kicker">变量监视</div>
          <div class="summary-value">{{ watchItems.length }}</div>
          <div class="summary-meta">个可查看节点结果</div>
        </div>
      </div>

      <NTabs type="line" animated size="small" class="tool-tabs">
        <NTabPane name="modules" tab="代码模块">
          <div class="panel-grid">
            <div class="left-list">
              <div class="list-title">模块索引</div>
              <div v-for="moduleId in modulePaths" :key="moduleId" class="module-item" @click="emit('load', moduleId)">
                ...
                <div class="module-desc">{{ modulesData[moduleId]?.exports?.length || 0 }} 个函数导出</div>
              </div>
            </div>
              ...
              <template v-if="modulesData[selectedModule]">
                <div class="pane-head">
                  <div>
                    ...
                    <div class="pane-subtitle">可直接编辑节点对应的 Python 模块代码</div>
                  </div>
                  <NSpace :size="8">
                    <NButton size="small" type="primary" @click="emit('saveModule', { id: selectedModule, source: moduleSource })">保存模块</NButton>
                    <NButton size="small" tertiary @click="emit('load', selectedModule)">重新载入</NButton>
                  </NSpace>
                </div>
        ...
        <NTabPane name="watch" tab="变量监视器">
          <NEmpty v-if="!watchItems.length" description="还没有可监视的节点结果" />
        ...
        <NTabPane name="env" tab="环境变量">
          <NEmpty v-if="!envVars.length" description="尚未配置环境变量" />
          <div v-else class="env-preview-list">
            <div v-for="(item, index) in envVars" :key="`${item.key}-${index}`" class="env-preview-row">
              <div>
                ...
                <div class="env-preview-meta">{{ item.secret ? '保密变量' : '普通变量' }}</div>
              </div>
